 // src/screens/split/SplitRequestOverlay.tsx
...
export default function SplitRequestOverlay({ onClose }: { onClose: () => void }) {
  ...
  // amount & note (✔ keep raw string to avoid cursor jump)
  ...
  // success / stickers (kept to minimize churn even though review step removed)
  const [stickers, setStickers] = useState<string[]>([]);
  const addSticker = (s: string) => setStickers((v) => [...v, s]);

  // Receipt summary state (generated on success)
  const [showPaymentSummary, setShowPaymentSummary] = useState(false);
  const [receiptRows, setReceiptRows] = useState<ReceiptRow[]>([]);

  // ⏱️ Auto-open Receipt Summary after 5s — MUST be declared before any early returns
  useEffect(() => {
    if (step === 5) {
      const t = setTimeout(() => setShowPaymentSummary(true), 5000);
          ...
          <div className="mx-auto grid max-w-3xl gap-6 md:grid-cols-2">
            ...
            <div className="rounded-2xl bg-emerald-50 p-4 ring-1 ring-emerald-200 md:col-span-2">
              ...
              <div className="mt-3 text-xs text-emerald-900/80">
                Tip: members will be taken from your current selection.
              </div>
            </div>
        ...
        <Page>
          <div className="mx-auto max-w-xl space-y-4">
            <div>
              ...
              <input
                type="text"              /* keep as text to avoid browser number quirks */
                autoFocus                /* keep focus stable while typing */
                value={amountInput}
        ...
        <Page>
          <div className="mx-auto max-w-3xl">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-700">
              <Users className="h-4 w-4" /> Select people
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-lg">
                        {p.avatar || "👤"}
                      </div>
  ...
  // STEP 3: allocation (✔ Replace "Review" with "Create split / Request money")
  if (step === 3) {
    return (
      <div className="fixed inset-0 z-50 grid place-items-center bg-black/50">
        <Page>
          <div className="mx-auto max-w-3xl">
              ...
              <div className="max-h-[48vh] overflow-y-auto">
                {members.map((m, i) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="grid grid-cols-[1fr_140px_120px] items-center border-t px-3 py-3"
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setMembers((ms) =>
                            ms.map((x, idx) => (idx === i ? { ...x, amount: v } : x))
                          );
                        }}
            ...
            <div className="mt-4 flex justify-between">
              ...
              {/* 🔽 Directly create split / request (no review step) */}
              <button
                onClick={() => {
                  // generate receipt rows for summary (mock "received" state)
                  const rows: ReceiptRow[] = members
                    .filter((m) => m.included && m.amount > 0 && m.id !== "me")
                    .map((m, idx) => ({
                      id: m.id,
                      requested: m.amount,
                      // Mock: first one paid, others pending
                      received: idx === 0 ? m.amount : 0,
                      receivedAt: idx === 0 ? Date.now() - 1000 * 60 * 45 : undefined, // 45 mins ago
                    }));
  ...
  return (
    <>
          ...
          {Array.from({ length: 70 }).map((_, i) => (
            <span
              ...
              style={{
                ...
                top: `-10px`,
                background:
                  i % 3 === 0 ? "#f59e0b" : i % 3 === 1 ? "#22c55e" : "#3b82f6",
                transform: `rotate(${(i * 17) % 360}deg)`,
                animation: `confettiFall ${1000 + (i % 6) * 400}ms ease-in ${
                  i * 30
                }ms both`,
              }}
            />
            ...
            <div className="mb-4 flex items-center gap-3 text-emerald-700">
              ...
              <div>
                ...
                <div className="text-sm text-emerald-700/90">
                  {mode === "split" ? "Split created" : "Request sent"} • Deep link ready to
                  share
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div className="rounded-2xl bg-amber-50 p-4 ring-1 ring-amber-200">
                ...
                <div className="text-xs text-amber-900/70">
                  Earn points for timely payments & active groups.
                </div>
              </div>
            </div>
      ...
      {showPaymentSummary && (
        <PaymentSummaryModal
          rows={receiptRows}
          totalRequested={members.filter(m=>m.included && m.id!=="me").reduce((s,m)=>s+m.amount,0)}
...
function PaymentSummaryModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/50">
      <div className="relative w-[min(520px,96vw)] rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/10">
        <button
          onClick={onClose}
        ...
        <div className="mb-4 text-[18px] font-bold text-slate-900">
          Payment Summary
        </div>

        <div className="mb-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-600">
          A quick report of who you asked, how much, and who paid.
        </div>

        <div className="space-y-2">
          {rows.map((r) => {
            const p = getPerson(r.id)!;
            const paid = r.received > 0;
            return (
              <div
                key={r.id}
              ...
              >
                ...
                <div className="text-right">
                  ...
                  >
                    {paid ? `Received: ${currency(r.received)}` : "Pending"}
                  </div>
                  {paid && r.receivedAt && (
        ...
        <div className="mt-4 rounded-xl bg-indigo-50 p-3 text-center text-[15px] font-semibold text-indigo-800">
          Total received: {currency(totalReceived)} / {currency(totalRequested)}
        </div>
      </div>
    </div>
  );
